...
exports.all = async (req, res) => {
  const articles = 
  await db.Article.find().populate('comments');
  return res.status("200").send(articles);
};
// Get one article
exports.one = async (req, res) => {
  const article = 
  await db.Article.find({ _id: req.params.id }).populate('comments');
...
exports.delete = async (req, res) => {
  const article = 
  await db.Article.deleteOne({ _id: req.params.id});
  return res.json(article);
};
// Return all articles that are saved as a favorite.
exports.allFavorites = async (req, res) => {
  const articles = 
  await db.Article.find({ favorite: true }).populate('comments');
  return res.status("200").send(articles);
};
// Scrape articles
exports.scrape = async (req, res) => {
  let newArticles = [];
  const scrape = 
  await axios.get("https://thenextweb.com/dd/");
  const $ = 
  await cheerio.load(scrape.data);
